# -*- coding: utf-8 -*-
"""
Created on Fri Mar 29 01:43:47 2024
...
from tools import tokenize, smiles2idx, normalize, pred_scatter_plot, denormalization, padding
...
class exp_mut:
    ...
    def load_autoencoders(self):
        ...
        self.autoencoder_exp = tf.keras.models.load_model(self.config.autoencoder_exp_file,custom_objects={'rmse': rmse,'r_square': r_square})
        self.autoencoder_mut = tf.keras.models.load_model(self.config.autoencoder_mut_file,custom_objects={'rmse': rmse,'r_square': r_square})
    ...
    def build(self):
        ...
        # Add Dense layers for mutation encoder
        for i, layer_units in enumerate(self.config.layers_enc_units[:-1]):
            encoder = tf.keras.layers.Dense(layer_units, activation="relu")(encoder)
            encoder = tf.keras.layers.BatchNormalization()(encoder)
            encoder = tf.keras.layers.Dropout(self.dropout)(encoder) 
    
        # Add output layer if layers_enc_units is not empty
        if self.config.layers_enc_units:
            encoder = tf.keras.layers.Dense(self.config.layers_enc_units[-1], activation="relu")(encoder)
        else:
            # If layers_enc_units is empty, create a default output layer with one unit
            encoder = tf.keras.layers.Dense(1, activation="relu")(encoder)
    
        # Create the model
        model_mut = tf.keras.Model(inputs=au_input, outputs=encoder)  
    
        # Load mutation encoder weights if autoencoders is set to True
        ...
        """Matrix nxn with mutation and expression information (multiplication of encoders)"""
        ...
        if self.config.ic50_imputed == 'False':
            ...
            x = tf.keras.layers.Concatenate()([global_pool_gene, model_drugs.output])
